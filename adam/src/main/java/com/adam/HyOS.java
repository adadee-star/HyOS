...
public class HyOS extends Application {
    ...
    private Node createSnakeGame() {
        ...
        c.setOnKeyPressed(e -> {
            if ((e.getCode() == KeyCode.W || e.getCode() == KeyCode.UP) && !dir[0].equals("DOWN"))
                dir[0] = "UP";
            if ((e.getCode() == KeyCode.S || e.getCode() == KeyCode.DOWN) && !dir[0].equals("UP"))
                dir[0] = "DOWN";
            if ((e.getCode() == KeyCode.A || e.getCode() == KeyCode.LEFT) && !dir[0].equals("RIGHT"))
                dir[0] = "LEFT";
            if ((e.getCode() == KeyCode.D || e.getCode() == KeyCode.RIGHT) && !dir[0].equals("LEFT"))
                dir[0] = "RIGHT";
        });
        ...
        new AnimationTimer() {
            ...
            public void handle(long now) {
                if (!running[0])
                    return;
                if (now - last < 1000_000_000 / speed[0])
                    return;
                last = now;
                ...
                if (dir[0].equals("UP"))
                    ...
                    nY++;
                else if (dir[0].equals("LEFT"))
                    nX--;
                else if (dir[0].equals("RIGHT"))
                    nX++;

                if (nX < 0 || nY < 0 || nX >= w / size || nY >= h / size)
                    running[0] = false;
                for (int[] p : snake)
                    if (p[0] == nX && p[1] == nY)
                        running[0] = false;
    ...
    private Node createAppStore() {
        ...
        b1.setOnAction(e -> {
            dock.getChildren().add(1, createLauncher("🕹️", "Snake", () -> spawnWindow("Snake", createSnakeGame())));
            b1.setDisable(true);
        });
        Button b2 = new Button("Install 📝 Notepad");
        b2.setOnAction(e -> {
            dock.getChildren().add(1, createLauncher("📝", "Notepad", () -> spawnWindow("Notepad", new TextArea())));
            b2.setDisable(true);
        });
        ...
        s.getChildren().addAll(new Label("Software Store"), b1, b2, b3);
        return s;
    }
